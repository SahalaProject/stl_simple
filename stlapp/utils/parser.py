        ...
        try:
            ...
            if level is 'test':
                ...
                if "skipReason" in body:
                    self.skip = body.pop('caseSkip')
                    self.skipReason = body.pop('skipReason')
...
class Parse(object):
    ...
    def __init__(self, body, level='test'):
        ...
        self.caseSkip = body.get('skip')
        self.skipReason = body.get('skipReason')
    ...
    def parse_http(self):
        ...
        if self.caseSkip:
            # 如果存在跳过的用例，初始test结构包含test，反之没有跳过的用例，走原有。
            #  初始化test结构
            test = {
                "name": self.name,

                "caseSkip": True,
                "skipReason": self.caseSkip,
